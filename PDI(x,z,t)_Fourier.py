...
@app.cell
def _(Nz, a_list, l_t, np, plt, t_list, x):
    # Plot
    #t,x,z
    plt.figure(figsize=(6, 4))
    sol = np.abs(np.array(a_list))
    cord_x = x.real.ravel()
    i_z = int(Nz/2)
    for j in range(1,l_t,10) :
        plt.plot(cord_x, sol[j,:,i_z], label=f"t = {t_list[j]}")
    #plt.xlim(0, Lx)
...
@app.cell
def _(Nx, l_t, plt, sol, t_list, z):
    cord_z = z.real.ravel()
    i_x = int(Nx/2)
    plt.figure(figsize=(6, 4))
    for i in range(1,l_t,10) :
        plt.plot(cord_z, sol[i,i_x, :], label=f"t = {t_list[i]}")
    #plt.xlim(0, Lx)
    #plt.ylim(0, time_stop)
    plt.xlabel('z')
    plt.ylabel('a')
